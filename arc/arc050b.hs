...
main = do
  (r,b) <- readInteger2 <$> BS.getLine
  (x,y) <- readInteger2 <$> BS.getLine
  print $ solve r b x y

solve r b x y = go n m  where
  sdiv p q = if p `mod` q == 0 then p `div` q else p `div` q
  m = (b*x-r) `sdiv` (x*y-1)
  n = (r*y-b) `sdiv` (x*y-1)
  go n m = if n*x+m <= r && n+m*y <= b then n+m
    else go (n-1) m `max` go n (m-1)
...
readInteger2 = toTuple . map readInteger . BS.words
