...
main = do
  ...
  ns <- map readInt642 <$> replicateM n BS.getLine
  print $ solve ns

-- solve = emulate 0 0 . sortBy (\(a,b) (c,d) -> compare (max a (a-b+c)) (max c (c-d+a)))
--   where
--     emulate t m [] = m
--     emulate t m ((a,b):xs) = emulate (t+a-b) (max m (t+a)) xs

solve ns = emulate 0 0
  $  (sortOn fst $ filter (\(a,b) -> a<b) ns)
  ++ (reverse . sortOn snd $ filter (\(a,b) -> a>=b) ns)
  where
    emulate t m [] = m
    emulate t m ((a,b):xs) = emulate (t+a-b) (max m (t+a)) xs
