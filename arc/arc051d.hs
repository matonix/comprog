...
main = do
  ...
  q <- readInt1 <$> BS.getLine
  qs <- map readInt2 <$> replicateM q BS.getLine
  print $ solve w h as bs qs

solve :: Int -> Int -> [Int] -> [Int] -> [(Int,Int)] -> [Int]
solve w h as bs qs = map query qs where
  aa = listArray (1,w) as
  bb = listArray (1,h) bs
  dp = listArray ((1,1),(w,h)) $ iter <$> range ((1,1),(w,h)) where
    iter (1,1) = aa!1+bb!1
    iter (i,1) = max (dp!(i-1,1)+aa!i+bb!1) (aa!i+bb!1)
    iter (1,j) = max (dp!(1,j-1)+aa!1+bb!j) (aa!1+bb!j)
    iter (i,j) = max (dp!(i,j-1)+dp!(i-1,j)-dp!(i-1,j-1)+aa!i+bb!j) (aa!i+bb!j)
  query (a,b) = dp!(a,b)
