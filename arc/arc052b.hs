...
main = do
  ...
  ns <- map readDouble3 <$> replicateM n BS.getLine
  qs <- map readDouble2 <$> replicateM q BS.getLine
  mapM_ print $ solve ns qs

solve :: [(Double,Double,Double)] -> [(Double,Double)] -> [Double]
solve ns qs = map (query ns) qs

query :: [(Double,Double,Double)] -> (Double,Double) -> Double
query ns (a,b) = volume (a,b) . fromJust $ find p ns where
  p (x,_,h) = not (x+h <= a || b <= x)

fst3 (a,_,_) = a

volume (a,b) (x,r,h)
  | x <= a && x+h <= b = cone (r*(h-a+x)/h) (h-a+x)
  | x >  a && x+h <= b = cone r h
  | x <= a && x+h >  b = tcone (r*(h-a+x)/h) (r*(h-b+x)/h) (b-a)
  | x >  a && x+h >  b = tcone r (r*(h-b+x)/h) (h-b+x)

cone !r !h = pi*r*r*h/3
tcone !r1 !r2 !h = pi*h*(r1*r1+r1*r2+r2*r2)/3
...
readDouble1 :: BS.ByteString -> Double
readDouble1 = fromIntegral . fst . fromJust . BS.readInt

readDouble2 :: BS.ByteString -> (Double,Double)
readDouble2 = toTuple . readDoubleN

readDouble3 :: BS.ByteString -> (Double,Double,Double)
readDouble3 = toTriple . readDoubleN

readDoubleN :: BS.ByteString -> [Double]
readDoubleN =  map readDouble1 . BS.words
