...
main = do
  ...
  xs <- map (\xs -> (head xs, length xs)) . group . removesparce . BS.unpack <$> BS.getLine
  putAns out . makeTable $ solve n k xs

removesparce [x] = [x]
removesparce (x:y:xs) = x:removesparce xs

solve :: Int -> Int -> [(Char,Int)] -> (Int,[Char])
solve n k xs = (0,[])

makeTable :: (Int,[Char]) -> Table
makeTable (c,cs) = [SingleR (IntC c), SingleR (ByteStringC (BL.intersperse ' ' . BL.pack $ cs))]
