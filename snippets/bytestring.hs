...
main = do
  a <- readInt1 <$> BS.getLine
  (b,c) <- readInt2 <$> BS.getLine
  s <- BS.getLine
  putAns out . makeTable $ solve a b c s

solve :: Int -> Int -> Int -> BS.ByteString -> (Int,BL.ByteString)
solve a b c s = (a+b+c,BL.fromStrict s)

makeTable :: (Int,BL.ByteString) -> Table
makeTable (abc,s) = [ TupleR( IntC(abc), ByteStringC(s) ) ]
