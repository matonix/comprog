...
main = do
  ...
  ns <- map readInt1 <$> replicateM n BS.getLine
  putAns out . makeTable $ solve ns

solve :: [Int] -> [Int]
solve ns = map (fromJust . flip M.lookup mp) ns where
  mp = M.fromAscList $ zip (S.toAscList . S.fromList $ ns) [0..]

makeTable :: [Int] -> Table
makeTable = map (\x -> SingleR (IntC x))
